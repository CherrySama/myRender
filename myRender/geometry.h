...
// 不同类型vec的相加
template<size_t DIM, typename T1, typename T2>vec<DIM, T1> operator+(vec<DIM, T1> lhs, const vec<DIM, T2>& rhs) {
    for (size_t i = DIM; i--; lhs[i] += rhs[i]);
    return lhs;
}
...
template <typename T> vec<3, T> cross(vec<2, T> v1, vec<2, T> v2) {
    return vec<3, T>(0, 0, v1.x * v2.y - v1.y * v2.x);
}
